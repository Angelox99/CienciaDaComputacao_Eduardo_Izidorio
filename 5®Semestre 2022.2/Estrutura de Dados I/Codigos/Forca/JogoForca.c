/* -----------------------------------------------------------------------------
Exercício: Ex 02 Structs – Jogo da Forca 
...
void imprime_tela_inicio(char vet_palavras[MAX_PALAVRAS][MAX_TAM_PALAVRA],
                         int qtd_palavras, Ranking *ranking)
{
  int escolha;
  do
  {
    ...
    printf("\nQual sua escolha: ");
    scanf("%d", &escolha);
    (void)getchar(); // limpar a entrada de nova linha
...
void add_ranking(Ranking *ranking, Jogador *jogador)
{
  ...
  if (!encontrou) ranking->vet_jogadores[ranking->num_jogadores++] = *jogador;
  for (int j = 1; j < ranking->num_jogadores; j++) {
...
void play_jogo(char vet_palavras[MAX_PALAVRAS][MAX_TAM_PALAVRA],
               int qtd_palavras, Ranking *ranking)
{
  ...
  while (1)
  {
    ...
    {
      printf("\nAcertou! - Palavra era: %s", p_correta);
      printf("\n--------- PARABENS !!!! ---------");
      printf("\n------- Você ZEROU o GAME! ------");
      printf("\n--- Sua pontuacao foi: %.2f ---", jogadorAtual->pontos);
      printf("\n---------------------------------\n");
      break;
    }
    ...
    while (1)
    {
      // 1. imprimir a forca
      limpa_tela();
      if (N_PAL_SORT > 1) printf("Acertou! - Palavra era: %s", p_correta);
      printf("\n#### JOGO FORCA ###\t\t Palavra[%d/%d]", N_PAL_SORT,
             qtd_palavras);
      imprime_forca(erros);
      ...
      if (strcmp(p_tela, p_sec) == 0)
      {
        ...
        jogadorAtual->pontos = jogadorAtual->pontos + (100 - (erros * 10)); // Aqui foi calculado uma proposta de pontuação
...
int le_arquivo(char vet_palavras[MAX_PALAVRAS][MAX_TAM_PALAVRA],
               int *qtd_palavras)
{
  FILE *file;
